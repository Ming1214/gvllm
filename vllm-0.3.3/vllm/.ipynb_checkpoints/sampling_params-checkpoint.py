...
class SamplingParams:
    ...
    def clone(self) -> "SamplingParams":
        ...
        if self.logits_processors is not None:
            for lp in self.logits_processors:
                memo[id(lp)] = lp
        if self.guidance_grammar is not None:
            memo[id(self.guidance_grammar)] = self.guidance_grammar
        if self.guidance_controller is not None:
            memo[id(self.guidance_controller)] = self.guidance_controller
        if len(memo) == 0: memo = None
