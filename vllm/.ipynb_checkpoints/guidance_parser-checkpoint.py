...
from guidance._grammar import Join, Select, Terminal, Null, Byte, ByteRange
...
class EarleyCommitParser:
    ...
    def consume_byte(self, byte, log_prob=0.0):
        ...
        for item in self.state_sets[self.state_set_pos + 1]:
            ...
            hidden_start = min(hidden_start, item.hidden_start)
        if not found_valid:
            raise Exception(f"Attempted to consume a byte ({byte}) that the grammar does not accept! Valid bytes: {self.valid_next_bytes()}")
